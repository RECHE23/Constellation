...
def linear_loss(input, target, reduction='mean'):
    if not (target.size() == input.size()):
        warnings.warn("Using a target size ({}) that is different to the input"
                      " size ({}). This will likely lead to incorrect results"
                      " due to broadcasting. Please ensure they have the same"
